...
export class ProductListQuery {
    ...
    _getFilterArgumentMap() {
        return {
            is_featured: is_featured => [`is_featured: { eq: ${is_featured} }`],
            ...
            lubricantRange: ({max, min}) => {
                const filters = [];
                if (min) filters.push(`min_lubricant_volume: { gteq: ${min} }`);
                if (max) filters.push(`max_lubricant_volume: { lteq: ${max} }`);
                return filters;
            },
            productsSkuArray: sku => [`sku: { in: [${sku.map(s => s.replace(/ /g, '%20'))}] }`],
    ...
    _getProductFields() {
        ...
        if (requireInfo) {
            return [
                'min_price',
                'max_price',
                'max_lubricant_volume',
                'min_lubricant_volume',
                this._getSortField(),
    ...
    _getUrlRewritesField(){
        return new Field('url_rewrites').addField('url')
    }
    ...
    _getOptionFields() {
        return new Fragment('CustomizableProductInterface')
            .addField(new Field('options')
                .addField(new Fragment('CustomizableDropDownOption')
                ...
                .addField(new Fragment('CustomizableCheckboxOption')
                    .addField(new Field('value')
                        .addFieldList([
                            'option_type_id',
                            'price',
