...
export const mapStateToProps = state => ({
    ...
    minLubricantRange: state.ProductListInfoReducer.min_lubricant_volume,
    maxLubricantRange: state.ProductListInfoReducer.max_lubricant_volume,
...
export class CategoryPageContainer extends PureComponent {
    ...
    config = {
        ...
        defaultLubricantRange: {min: 0, max: 300},
        sortKey: 'name',
        sortDirection: 'ASC'
    };
    ...
    containerFunctions = {
        ...
        updateLubricantRange: this.updateLubricantRange.bind(this)
    };
    ...
    containerProps = () => ({
        ...
        selectedLubricantRange: this._getLubricantRangeForSlider()
    });
    ...
    updateLubricantRange(lubricantRange) {
        const {location, history} = this.props;
        setQueryParams({
            lubricantMax: lubricantRange.max,
            lubricantMin: lubricantRange.min,
            page: ''
        }, location, history);
    }
    ...
    _getSelectedLubricantRangeFromUrl() {
        const {location} = this.props;
        const min = +getQueryParam('lubricantMin', location);
        const max = +getQueryParam('lubricantMax', location);
        return {min, max};
    }
    ...
    _getLubricantRangeForSlider() {
        const {minLubricantRange, maxLubricantRange} = this.props;
        const {defaultLubricantRange: {min: defaultMin, max: defaultMax}} = this.config;
        const {min, max} = this._getSelectedLubricantRangeFromUrl();
        return {min: min || minLubricantRange || defaultMin, max: max || maxLubricantRange || defaultMax};
    }
    ...
    _getFilter() {
        ...
        const lubricantRange = this._getSelectedLubricantRangeFromUrl();
