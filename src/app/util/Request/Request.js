...
const GRAPHQL_URI = process.env.API_URL + '/graphql';
...
const appendTokenToHeaders = (headers) => {
    ...
    const Store = {Store: 'ru'};
    const current = BrowserDatabase.getItem('current_store');
    if (current) {
        Store.Store = current.store_code
    }
    return {
        ...headers,
        ...Store,
...
const current_store = (requestUrl) => {
    let store = requestUrl;
    const current = BrowserDatabase.getItem('current_store');
    if (current) {
        store = store + ((store.indexOf('?') === -1 ? '?' : '&') + `langCache=${current.store_code}`)
    }
    return store;
};
...
const formatURI = (query, variables, url) => {
    ...
    return current_store(requestUrl);
};
